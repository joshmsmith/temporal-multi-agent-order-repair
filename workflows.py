...
class RepairAgentWorkflowSharingContext(RepairAgentWorkflow):
    ...
    async def run(self, inputs: dict) -> str:
        ...
        #todo load orders and inventory data into context here for all agents to use <-- probably a bad plan since these can change over time
        ...
        #TODO make the report agent use the shared context instead of its own limited/refreshed context <--- this is also a bad idea, can cause context poisoning
        # Create the report with the report agent
        report_summary = await self.generate_report()
